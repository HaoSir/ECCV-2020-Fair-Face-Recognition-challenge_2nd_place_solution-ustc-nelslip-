...
def make_weights_for_balanced_classes_old(images, nclasses):
    '''
    ...
    return weight
def make_weights_for_balanced_classes(images, classes):
    '''
        Make a vector of weights for each image in the dataset, based
        on class frequency. The returned vector of weights can be used
        to create a WeightedRandomSampler for a DataLoader to have
    ...
    for idx, val in enumerate(images):
        weight[idx] = weight_per_class[val[1]]
    subject_skingender={}
    with open('train_gtruth_new.csv','r') as f:
        lines = f.readlines()[1:]
        for line in lines:
            subject=line.strip().split(',')[0]
            skingender=line.strip().split(',')[1]
            subject_skingender[subject]=skingender
    skingender_rate={'0':1.964,'1':3.380,'2':7.870,'3':14.707}
    for idx, val in enumerate(images):
        weight[idx] = weight[idx]*skingender_rate[subject_skingender[classes[val[1]]]]
    return weight
...
def perform_val_old(multi_gpu, device, embedding_size, batch_size, backbone, carray, issame, nrof_folds = 5, tta = True):
    if multi_gpu:
    ...
    roc_curve.save('roc.jpg')
    roc_curve_tensor = transforms.ToTensor()(roc_curve)

    return accuracy.mean(), best_thresholds.mean(), roc_curve_tensor,val.mean(), val_std.mean(), far.mean()
